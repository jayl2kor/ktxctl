...
로그인 성공 시 `로그인완료!`를 표시하고 Enter 입력을 기다립니다.
로그인 설정과 저장된 계정 로그인 모두 적용되며, 최초 계정 설정 후 자동 로그인에서는
한 번만 확인합니다. 로그인 실패·입력 취소 시에는 완료 확인을 표시하지 않습니다.

## 코레일 전용 구성

SRT 클라이언트와 사업자 선택 메뉴를 제거했습니다.
...
`~/srtgo/srtgo/srtgo.py`의 기존 SRT 역 목록을 코레일 역 목록과 통합해
총 45개 역을 제공합니다. 기존 순서를 유지하고 다음 13개 역을 뒤에 추가했습니다.

수서, 동탄, 평택지제, 곡성, 공주, 구례구, 나주, 남원, 서대구, 여천, 진영, 진주, 창원.

`김천(구미)`는 기존 `김천구미` 항목으로 통합합니다. 기존 즐겨찾기와 기본 선택은
유지됩니다. 새 역은 `역 설정`에서 선택·저장하면 출발역·도착역 선택에 표시됩니다.
