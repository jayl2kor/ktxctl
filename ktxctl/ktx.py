...
from .ktx_auth import AuthHeaders, DYNAPATH_PATHS, DynaPathMasterEngine
...
class Korail:
    ...
    def _get_auth_headers_and_sid(self, url: str) -> tuple[AuthHeaders, str | None]:
        headers: AuthHeaders = {}
        sid = None
        if any(path in url for path in DYNAPATH_PATHS):
            ts = int(time.time() * 1000)
            rand = "".join(random.choices(string.ascii_uppercase + string.digits, k=4))
            headers["x-dynapath-m-token"] = self._engine.generate_token(self._device_id, ts, rand)
    ...
    def search_train(
        ...
        r = self._session.post(API_ENDPOINTS["search_schedule"], params=data, headers=headers)
        self._log(r.text)
        j = json.loads(r.text)
    ...
    def reserve(self, train, passengers=None, option=ReserveOption.GENERAL_FIRST):
        ...
        headers, _ = self._get_auth_headers_and_sid(API_ENDPOINTS["reserve"])
