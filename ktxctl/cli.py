...
@click.command(name="ktxctl")
@click.option("--debug", is_flag=True, help="Debug mode")
def ktxctl(debug=False):
    MENU_CHOICES = [
        ("예매 시작", 1),
        ("예매 확인/결제/취소", 2),
        ("로그인 설정", 3),
    ...
    ACTIONS = {
        1: lambda: reserve(debug=debug),
        2: lambda: check_reservation(debug=debug),
        3: lambda: set_login(debug=debug),
        4: set_telegram,
        5: set_card,
        6: set_station,
        7: edit_station,
...
def set_station() -> bool:
    ...
    keyring.set_password(
        "KTX", "station", (selected_stations := ",".join(selected))
    )
...
def edit_station() -> bool:
    station_info = inquirer.prompt(
        [
            inquirer.Text(
                "stations",
                message="역 수정 (예: 서울,대전,동대구)",
                default=keyring.get_password("KTX", "station") or "",
            )
        ]
    )
    if not station_info:
...
def set_options():
    ...
    keyring.set_password("KTX", "options", ",".join(options))


def get_options():
    options = keyring.get_password("KTX", "options") or ""
...
def set_telegram() -> bool:
    ...
    try:
        ...
        asyncio.run(tgprintf("[KTXCTL] 텔레그램 설정 완료"))
        return True
...
def set_login(debug: bool = False, *, pause: bool = True) -> bool:
    ...
    login_info = inquirer.prompt(
        [
            inquirer.Text(
                "id",
                message="KTX 계정 아이디 (멤버십 번호, 이메일, 전화번호)",
    ...
    try:
        ...
        print(err)
        if keyring.get_password("KTX", "ok") is not None:
            keyring.delete_password("KTX", "ok")
        return False


def login(debug: bool = False) -> Korail | None:
    if (
    ...
    password = keyring.get_password("KTX", "pass")
...
def reserve(debug: bool = False) -> None:
    ...
    defaults = {
        "departure": keyring.get_password("KTX", "departure") or "서울",
        "arrival": keyring.get_password("KTX", "arrival") or "동대구",
        "date": keyring.get_password("KTX", "date") or today,
        "time": keyring.get_password("KTX", "time") or "120000",
        "adult": int(keyring.get_password("KTX", "adult") or 1),
        "child": int(keyring.get_password("KTX", "child") or 0),
        "senior": int(keyring.get_password("KTX", "senior") or 0),
        "disability1to3": int(keyring.get_password("KTX", "disability1to3") or 0),
        "disability4to6": int(keyring.get_password("KTX", "disability4to6") or 0),
    }

    # Set default stations if departure equals arrival
    if defaults["departure"] == defaults["arrival"]:
        defaults["arrival"] = (
            "동대구" if defaults["departure"] == "서울" else None
        )
    ...
    for key, value in info.items():
        keyring.set_password("KTX", key, str(value))
...
def _is_seat_available(train: Train, seat_type: str) -> bool:
...
def check_reservation(debug: bool = False) -> None:
    rail = login(debug=debug)
    if rail is None:
        return
